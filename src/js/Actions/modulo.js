...
//VISTA QUE PERTENECE -> views/modulos/index.php
    ...
    function verModulo(id) {
        $.ajax({
            ...
            url: 'web.php?controller=modulo&action=view&id='+id,
            method: 'GET',
            dataType: 'json',
            
            success: function (data) {
                if (data.success) {
                    document.getElementById('descripcion_read').value = data.modulo.descripcion;
                    document.getElementById('estado_read').value = data.modulo.estado;

                    var modal = new bootstrap.Modal(document.getElementById('modulo_read'));
                    modal.show();
                } else {
    ...
    //METODO PARA CREAR
    document.querySelector('.btn-moduloCreate').addEventListener('click', function() {
        ...
        fetch('web.php?controller=modulo&action=store', {
                method: 'POST',
                body: formData
            })
            ...
            .then(data => {
                if (data === 'success') {
                    Swal.fire({
                        icon: 'success',
                        title: '¡Exito!',
                        text: 'Nuevo Modulo guardado en la BD!',
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: '¡Error!',
                        text: 'No se pudo Guardar el Módulo',//data,
                        confirmButtonColor: '#0072ff'
                    }).then((result) => {
                        if (result.isConfirmed) {
                            location.reload();
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.success) {
                    ...
                    document.getElementById('descripcion_update').value = data.modulo.descripcion;
                    document.getElementById('estado_update').value = data.modulo.estado;

                    var modal = new bootstrap.Modal(document.getElementById('modulo_update'));
                    modal.show();
                } else {
                    alert('No se encontró el módulo.');
                }
    ...
    //METODO PARA EDITAR
    document.querySelector('.btn-moduloUpdate').addEventListener('click', function() {
        ...
        Swal.fire({
            icon: 'question',
            title: '¿Estás seguro?',
            text: '¿Deseas actualizar este módulo?',
            ...
            if (result.isConfirmed) {
                ...
                fetch('web.php?controller=modulo&action=update', {
                        method: 'POST',
                        body: formData
                    })
                    .then(response => response.text())
            ...
            success: function(data) {
                if (data.success) {
                    Swal.fire({
                        icon: 'question',
                        title: '¿Estás seguro?',
                        text: '¿Deseas eliminar este módulo?',
                        showCancelButton: true,
                        ...
                        if (result.isConfirmed) {
                            ...
                            // Enviar datos a PHP
                            const formData = new FormData();
                            formData.append('id', data.modulo.id);

                            // función para eliminar
                            fetch('web.php?controller=modulo&action=delete', {
                                    method: 'POST',
                                    body: formData
                                })
                                .then(response => response.text())
                                .then(data => {
                                    if (data === 'success') {
                                        ...
                                        Swal.fire({
                                            icon: 'success',
                                            title: '¡Exito!',
                                            text: 'Modulo Eliminado en la BD!',
                                            confirmButtonColor: '#0072ff'
                                        }).then((result) => {
                                            if (result.isConfirmed) {
                                                location.reload();
                                        ...
                                        Swal.fire({
                                            icon: 'error',
                                            title: '¡Error!',
                                            text: 'No se pudo Eliminar el Módulo',//data,
                                            confirmButtonColor: '#0072ff'
                                        }).then((result) => {
                                            if (result.isConfirmed) {
                                                location.reload();
